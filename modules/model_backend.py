...
def predict_triage(symptoms, model, feature_cols):
    ...
    if orange_result == "orange":
        return {
            "color": "orange",
            "source": "Warning rule",
            "message": "Symptoms need medical review within 24-48 hours."
        }
    active_symptoms = [
    ...
    if len(active_symptoms) < 2:
     return {
        ...
        "message": "Too few symptoms to assess. Visit a clinic within 48 hours."
    }
    # ML model third
    ...
    if color == "red" and confidence < 95:
        color = "orange"
    return {
        "color": color,
        "source": "Machine learning model",
        "message": f"Prediction generated from selected symptoms. Confidence: {confidence}%"
     }