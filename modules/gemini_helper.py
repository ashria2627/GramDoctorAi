...
def get_active_symptoms(symptoms):
    ...
    return ", ".join(active)
def extract_diagnosis(ai_response_text):
    if not ai_response_text:
        return None
    for line in ai_response_text.split("\n"):
        if line.strip().startswith("Probable Diagnosis:"):
            diagnosis = line.replace("Probable Diagnosis:", "").strip()
...
- For Probable Diagnosis, return only the condition name in English, exactly as a doctor would write it. One condition only.
