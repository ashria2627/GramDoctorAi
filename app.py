...
from modules.gemini_helper import generate_ai_response,extract_diagnosis
import pandas as pd
...
with tab1:
    ...
    if search_term:
      filtered_symptoms = [
        symptom for symptom in sorted_symptoms
        if search_term.lower() in symptom.lower()
      ]
    else:
      filtered_symptoms = sorted_symptoms
    ...
    if st.session_state.triage_result is None:
        ...
        if color == "gray":
            ...
            if st.session_state.ai_response:
                ...
                diagnosis = extract_diagnosis(st.session_state.ai_response)
                if diagnosis:
                   diag_df = pd.read_csv("assets/diagnosis_to_triage_color_map.csv")
                   match = diag_df[diag_df['diagnosis'].str.lower() == diagnosis.lower()]
                   if not match.empty:
                     confirmed_color = match['triage_color'].values[0]
                     st.info(f"🔍 Most likely condition: **{diagnosis}** — confirmed **{confirmed_color.upper()}** in medical database")
                   else:
                     st.info(f"🔍 Most likely condition: **{diagnosis}**")
